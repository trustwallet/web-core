import axios from 'axios';
import { getEnv } from '../utils';
import { BitcoinRPC } from './BitcoinRPC';
import { BitcoinFeePriority } from './models';

describe('BitcoinRPC', () => {
    let rpc: BitcoinRPC;
    const xpub =
        'xpub6CUGRUonZSQ4TWtTMmzXdrXDtypWKiKrhko4egpiMZbpiaQL2jkwSB1icqYh2cfDfVxdx4df189oLKnC5fSwqPfgyP3hooxujYzAu3fDVmz';
    const address = '1F1tAaz5x1HUXrCNLbtMDqcw6o5GNn4xqX';
    const tx = '372f2f8f815fdb885bc1e6c811c9a4cc9af769e5288f926d7b975e44b6f6cef6';

    beforeAll(() => {
        require('dotenv').config({ path: __dirname + '/./.env' });
        rpc = new BitcoinRPC(getEnv('BTC_RPC_URL'));
    });

    beforeEach(function() {
        require('dotenv').config({ path: __dirname + '/./.env' });
        spyOn(axios, 'get').and.returnValue({ data: { backend: {} } });
        spyOn(axios, 'post').and.returnValue({ data: { backend: {} } });
    });

    it('should get last block', async () => {
        const addrToBeCalled = `${getEnv('BTC_RPC_URL')}/v2/`;
        await rpc.getLatestBlock();
        expect(axios.get).toHaveBeenCalledWith(addrToBeCalled);
    });

    it('should get balance', async () => {
        await rpc.getBalance(xpub);
        const addToBeCalled = `${getEnv('BTC_RPC_URL')}/v2/xpub/${xpub}?tokens=used`;
        expect(axios.get).toHaveBeenCalledWith(addToBeCalled);
    });

    it('should get listUnspent', async () => {
        await rpc.listUnspent(xpub);
        const addToBeCalled = `${getEnv('BTC_RPC_URL')}/v2/utxo/${xpub}`;
        expect(axios.get).toHaveBeenCalledWith(addToBeCalled);
    });

    it('should get listTransactions', async () => {
        await rpc.listTransactions(address);
        const addToBeCalled = `${getEnv('BTC_RPC_URL')}/v2/address/${address}`;
        expect(axios.get).toHaveBeenCalledWith(addToBeCalled);
    });

    it('should get tx detail', async () => {
        await rpc.getTransactionDetail(tx);
        const addToBeCalled = `${getEnv('BTC_RPC_URL')}/v2/tx/${tx}`;
        expect(axios.get).toHaveBeenCalledWith(addToBeCalled);
    });

    it('should get estimated fee', async () => {
        await rpc.estimateFee(BitcoinFeePriority.FASTEST);
        const addToBeCalled = `${getEnv('BTC_RPC_URL')}/v2/estimatefee/${BitcoinFeePriority.FASTEST}`;
        expect(axios.get).toHaveBeenCalledWith(addToBeCalled);
    });

    it('should get broadcast transaction', async () => {
        const data =
            '';
        await rpc.broadcastTransaction(data);
        expect(axios.post).toHaveBeenCalled();
    });
});
